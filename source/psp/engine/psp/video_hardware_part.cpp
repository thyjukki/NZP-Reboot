...
particle2_t	*active_particles, *free_particles;
...
===============
*/
void R_Init_Classic_Particles (void)
{
	int		i;
...
===============
*/
void R_Classic_LavaSplash (vec3_t org)
{
	int			i, j, k;
	particle2_t	*p;
	float		vel;
...
void R_Rocket_Classic_Trail (vec3_t start, vec3_t end, int type)
{
	vec3_t		vec;
	float		len;
	int			j;
...
void R_RocketTrail (vec3_t start, vec3_t end, int type)
{
	...
	if (qmb_initialized && r_part_trails.value)
		...
		R_Rocket_Classic_Trail (start, end, type);
}
